 ...
 */
#pragma once
#include "tm_qmm_mfma.cuh"           // load_wfrag/load_xfrag/mma16816, fp8_raw
#include "quant_formats.cuh"    // e4m3/e2m1 decode+encode, nvfp4_sf_offset
#include "tm_warp.cuh"        // warp_max_f (activation/experts quant)
...
// Store one MFMA 16x16 accumulator tile with a per-output-column (n) scale.
// MFMA layout: lane l owns output column n = n0 + l%16 and rows
// m0 + 4*(l/16) + {0..3}, so a single column scale applies to all 4 regs.
__device__ __forceinline__ void store_scaled(float* Y, int N, int total_rows, int m0, int n0,
                                             const float4_t& acc, float sN) {
    const int l = threadIdx.x & 63;
    const int n = n0 + (l & 15);
    const int mrow = m0 + (l >> 4) * 4;
    #pragma unroll
    for (int v = 0; v < 4; v++)
        if (mrow + v < total_rows) Y[size_t(mrow + v) * N + n] = acc[v] * sN;
}
...
__global__ void moe_gemm_fp8(float* __restrict__ Y, const half* __restrict__ A,
    ...
    for (int k0 = 0; k0 < K; k0 += 16) {
        ...
        half4_t b  = load_wfrag<fp8_raw>(Be, bpr, n0, k0);   // dequant once, reused by aT and aB
        accT = mma_16x16x16(aT, b, accT);
        accB = mma_16x16x16(aB, b, accB);
    }
    const float sN = Se[n0 + (threadIdx.x & 15)];
    store_scaled(Y, N, total_rows, m0,      n0, accT, sN);
...
// MFMA: lane l holds row m=m0+l%16 (loc lr=loc0+l%16), K = k0 + 4*(l/16) + {0..3},
// all inside one 16-group (group = k0/16), so a single A-scale for the fragment.
__device__ __forceinline__ half4_t load_afrag_nvfp4(const uint8_t* A,
        const uint8_t* Asc_e, int K, int num_k_tiles, int m0, int loc0, int k0) {
    const int l = threadIdx.x & 63;
    const int rr = m0 + (l & 15), lrr = loc0 + (l & 15);
    const int cc = k0 + (l >> 4) * 4;
    const int Kh = K / 2, group = cc / 16;
    const float as = e4m3_decode(Asc_e[nvfp4_sf_offset(lrr, group, num_k_tiles)]);
    const uint8_t b0 = A[(size_t)rr * Kh + cc / 2];        // cols cc,   cc+1
    const uint8_t b1 = A[(size_t)rr * Kh + cc / 2 + 1];    // cols cc+2, cc+3
    half4_t f;
    f[0] = (__fp16)(e2m1_decode(b0 & 0xF) * as);
    f[1] = (__fp16)(e2m1_decode(b0 >> 4)  * as);
    f[2] = (__fp16)(e2m1_decode(b1 & 0xF) * as);
    f[3] = (__fp16)(e2m1_decode(b1 >> 4)  * as);
    return f;
}
...
// MFMA: lane l holds W row n = n0 + l%16, K = k0 + 4*(l/16) + {0..3} (one group).
__device__ __forceinline__ half4_t load_wfrag_nvfp4(const uint8_t* B,
        const uint8_t* Bsc_e, int K, int groups, int n0, int k0) {
    const int l = threadIdx.x & 63;
    const int rr = n0 + (l & 15), cc = k0 + (l >> 4) * 4;
    const int Kh = K / 2, group = cc / 16;
    const float bs = e4m3_decode(Bsc_e[(size_t)rr * groups + group]);
    const uint8_t b0 = B[(size_t)rr * Kh + cc / 2];
    const uint8_t b1 = B[(size_t)rr * Kh + cc / 2 + 1];
    half4_t f;
    f[0] = (__fp16)(e2m1_decode(b0 & 0xF) * bs);
    f[1] = (__fp16)(e2m1_decode(b0 >> 4)  * bs);
    f[2] = (__fp16)(e2m1_decode(b1 & 0xF) * bs);
    f[3] = (__fp16)(e2m1_decode(b1 >> 4)  * bs);
    return f;
}
...
__global__ void moe_gemm_nvfp4(float* __restrict__ Y, const uint8_t* __restrict__ A,
    ...
    float4_t accT = {0, 0, 0, 0}, accB = {0, 0, 0, 0};
    for (int k0 = 0; k0 < K; k0 += 16) {
        half4_t aT = load_afrag_nvfp4(A, Asc_e, K, num_k_tiles, m0,      loc0,      k0);
        half4_t aB = load_afrag_nvfp4(A, Asc_e, K, num_k_tiles, m0 + 16, loc0 + 16, k0);
        half4_t b  = load_wfrag_nvfp4(Be, Bsc_e, K, groups, n0, k0);   // dequant once, reused
        accT = mma_16x16x16(aT, b, accT);
        accB = mma_16x16x16(aB, b, accB);
    }
    const float al = alphas[e];
    store_scaled(Y, N, total_rows, m0,      n0, accT, al);
    store_scaled(Y, N, total_rows, m0 + 16, n0, accB, al);
}
...
template <int BIT>
__device__ __forceinline__ half4_t load_wfrag_wna16(const uint32_t* qw,
    ...
    const int l = threadIdx.x & 63;
    const int rr = n0 + (l & 15), c = k0 + (l >> 4) * 4;   // 4 contiguous K per lane
    const size_t nb = (size_t)rr * packed_k;
    half4_t f;
    #pragma unroll
    for (int v = 0; v < 4; v++) {
        const int cc = c + v;
        const int g = cc / group_size;
        const float s = __half2float(scales[(size_t)rr * groups + g]);
        f[v] = (__fp16)wna16_deq<BIT>(qw, nb, cc, qz, 0, rr, g, groups, has_zp, s);
    }
    return f;
}
...
__global__ void moe_gemm_wna16(float* __restrict__ Y, const half* __restrict__ A,
    ...
    for (int k0 = 0; k0 < K; k0 += 16) {
        half4_t aT = load_xfrag(A, K, m0, k0);
        half4_t aB = load_xfrag(A, K, m0 + 16, k0);
        half4_t b  = load_wfrag_wna16<BIT>(qw, qz, sc, N, K, groups, group_size, has_zp != 0, n0, k0);
        accT = mma_16x16x16(aT, b, accT);
        accB = mma_16x16x16(aB, b, accB);
    }
