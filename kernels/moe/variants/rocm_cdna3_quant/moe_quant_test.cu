...
int main() {
    // ---- fp8 MoE GEMM ----
    {
        ...
        moe_gemm_fp8<<<grid, 64>>>(dY, reinterpret_cast<const half*>(dA), dB, dSc, dEot,
                                   total_rows, N, K);
    ...
    // ---- nvfp4 dual-fp4 MoE GEMM ----
    {
        ...
        float* dY = dzero<float>((size_t)total_rows * N);
        dim3 grid(N / 16, total_rows / 32);
        moe_gemm_nvfp4<<<grid, 64>>>(dY, dAb, dBb, dAsc, dBsc, dal, deot, der0, dsfo,
                                     total_rows, N, K);
        CK(hipDeviceSynchronize());
        if (hipGetLastError() != hipSuccess) { printf("NVFP4 KERNEL ERR\n"); return 1; }
        auto Y = d2h(dY, (size_t)total_rows * N);
    ...
    for (int BIT : {4, 8}) {
        ...
        dim3 grid(N / 16, total_rows / 32);
        if (BIT == 4)
            moe_gemm_wna16<4><<<grid, 64>>>(dY, reinterpret_cast<const half*>(dA), dqw,
                reinterpret_cast<const half*>(dsc), dqz, dEot, total_rows, N, K, GS, 1);
        else
            moe_gemm_wna16<8><<<grid, 64>>>(dY, reinterpret_cast<const half*>(dA), dqw,
                reinterpret_cast<const half*>(dsc), dqz, dEot, total_rows, N, K, GS, 1);
        CK(hipDeviceSynchronize());
        if (hipGetLastError() != hipSuccess) { printf("WNA16 KERNEL ERR\n"); return 1; }
        auto Y = d2h(dY, (size_t)total_rows * N);
        double gsum = 0, rsum = 0;
        for (int r = 0; r < total_rows; ++r) {
            const int e = eot[r / 32];
