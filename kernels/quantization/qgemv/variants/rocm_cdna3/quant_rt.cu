...
int main() {
    ...
    hipMalloc(&dA, n * 4); hipMalloc(&dC, n); hipMalloc(&dS, sizeof(float) * T); hipMalloc(&dM, 4);
    ...
    auto run = [&](bool fp8, bool per_tensor) {
        ...
        hipMemcpy(C.data(), dC, n, hipMemcpyDeviceToHost);
        hipMemcpy(S.data(), dS, sizeof(float) * T, hipMemcpyDeviceToHost);
        const float QMAX = fp8 ? 448.0f : 127.0f;
