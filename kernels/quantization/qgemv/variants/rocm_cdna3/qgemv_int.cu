...
int main(int argc, char** argv) {
    ...
    // Checked-in golden_int omits meta.txt; default to gen_golden_int.py dims
    // (N=512, K=4096). Override with: <dir> [N K].
    int N = 512, K = 4096;
    FILE* f = fopen((dir + "/meta.txt").c_str(), "r");
    if (f) {
        if (fscanf(f, "%d %d", &N, &K) != 2) { fprintf(stderr, "bad meta\n"); return 2; }
        fclose(f);
    } else if (argc >= 4) {
    ...
    { // W8A8
        ...
        hipMalloc(&dW, Wq.size()); hipMalloc(&dX, Xq.size());
        hipMalloc(&dws, ws.size()); hipMalloc(&das, as.size()); hipMalloc(&dD, sizeof(half) * N);
        hipMemcpy(dW, Wq.data(), Wq.size(), hipMemcpyHostToDevice);
        hipMemcpy(dX, Xq.data(), Xq.size(), hipMemcpyHostToDevice);
        hipMemcpy(dws, ws.data(), ws.size(), hipMemcpyHostToDevice);
        hipMemcpy(das, as.data(), as.size(), hipMemcpyHostToDevice);
        ...
        float ms; hipEventElapsedTime(&ms, t0, t1); ms /= 200;
        std::vector<half> got(N);
        hipMemcpy(got.data(), dD, sizeof(half) * N, hipMemcpyDeviceToHost);
    ...
    { // W2A8 bitnet
        ...
        hipMalloc(&dW, Wq.size()); hipMalloc(&dX, Xq.size());
        hipMalloc(&das, as.size()); hipMalloc(&dD, sizeof(half) * N);
        hipMemcpy(dW, Wq.data(), Wq.size(), hipMemcpyHostToDevice);
        hipMemcpy(dX, Xq.data(), Xq.size(), hipMemcpyHostToDevice);
        hipMemcpy(das, as.data(), as.size(), hipMemcpyHostToDevice);
        hipEvent_t t0, t1; hipEventCreate(&t0); hipEventCreate(&t1);
