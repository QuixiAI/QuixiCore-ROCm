...
int run(const std::string& dir, int N, int K) {
    ...
    hipMalloc(&dWdeq, sizeof(float) * N * K);
    hipMalloc(&dX, sizeof(half) * K);
    hipMalloc(&dD, sizeof(half) * N);
    ...
    hipMemcpy(wgot.data(), dWdeq, sizeof(float) * N * K, hipMemcpyDeviceToHost);
    ...
    hipMemcpy(dg.data(), dD, sizeof(half) * N, hipMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
    ...
    // meta: "fmt N K". The checked-in golden omits meta.txt, so fall back to the
    // directory basename for the format and the gen_golden.py fixed dims
    // (N=512, K=4096). Override with: <dir> [N K].
    char fmt[64]; int N = 512, K = 4096;
    {
        FILE* f = fopen((dir + "/meta.txt").c_str(), "r");
        if (f) {
            if (fscanf(f, "%63s %d %d", fmt, &N, &K) != 3) { fprintf(stderr, "bad meta\n"); return 2; }
            fclose(f);
        } else {
            std::string base = dir;
            ...
            snprintf(fmt, sizeof(fmt), "%s", name.c_str());
