...
template <typename T> T* dz(size_t n){T*d;CK(hipMalloc(&d,n*sizeof(T)));CK(hipMemset(d,0,n*sizeof(T)));return d;}
...
static void test_fwht() {
    ...
    fwht_rotate<float,D,false><<<R,32>>>(dx,dfwd,dsg,R); CK(hipDeviceSynchronize());
    fwht_rotate<float,D,true><<<R,32>>>(dfwd,dinv,dsg,R); CK(hipDeviceSynchronize());
...
int main() {
    ...
    { const int R=40, C=256; auto x=rv((size_t)R*C,-1,1);
      ...
      dim3 g((C+255)/256,R); permute_cols<uint16_t><<<g,256>>>(dx,do_,dp,R,C); CK(hipDeviceSynchronize());
    ...
    { const int max_loras=3, E=8, topk=2, block=4, T=24;
      ...
      if (hipGetLastError()!=hipSuccess){printf("LORA KERNEL ERR\n");return 1;}
