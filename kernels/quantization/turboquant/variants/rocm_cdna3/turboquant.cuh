...
template <> __device__ __forceinline__ float t6<__hip_bfloat16>(__hip_bfloat16 v) { return __bfloat162float(v); }
...
template <> __device__ __forceinline__ __hip_bfloat16  f6<__hip_bfloat16>(float v) { return __float2bfloat16(v); }
...
__global__ void fwht_rotate(const T* __restrict__ x, T* __restrict__ out,
    ...
    for (int m = 1; m < 32; m <<= 1) {
        ...
        #pragma unroll
        for (int i = 0; i < E; ++i) {
            const float p = __shfl_xor(v[i], m);
...
__device__ __forceinline__ float tg_forward_fwht(float x, float* buf, const float* sign, int t) {
    ...
    for (int s = 0; s < 5 && s < NS; ++s) {
        ...
        const float p = __shfl_xor(x, mask);
        x = (t & mask) ? (p - x) : (x + p);
    }
    if (NS > 5) {
...
__global__ void tq_encode(const T* __restrict__ key, const T* __restrict__ value,
    ...
    for (int o = 16; o > 0; o >>= 1) {
        k_min = fminf(k_min, __shfl_xor(k_min, o));
        k_max = fmaxf(k_max, __shfl_xor(k_max, o));
    }
    ...
    #pragma unroll
    for (int o = 16; o > 0; o >>= 1) v_sq += __shfl_xor(v_sq, o);
