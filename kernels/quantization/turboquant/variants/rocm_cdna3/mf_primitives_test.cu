...
static int g_fail = 0;
#define CK(x) do { hipError_t e=(x); if(e){printf("CUDA %s @%d\n",hipGetErrorString(e),__LINE__);exit(1);} } while(0)
...
int main() {
    ...
    CK(hipMalloc(&din, N*4)); CK(hipMalloc(&ddeq, N*4)); CK(hipMalloc(&dcode, N));
    CK(hipMemcpy(din, in.data(), N*4, hipMemcpyHostToDevice));
    fp4_roundtrip<<<(N+255)/256,256>>>(din, ddeq, dcode, N);
    CK(hipDeviceSynchronize());
    std::vector<float> deq(N); CK(hipMemcpy(deq.data(), ddeq, N*4, hipMemcpyDeviceToHost));
    ...
    float *dp, *dpo; CK(hipMalloc(&dp, 60*4)); CK(hipMalloc(&dpo, 60*4));
    CK(hipMemcpy(dp, pin.data(), 60*4, hipMemcpyHostToDevice));
    e8m0_roundtrip<<<1,64>>>(dp, dpo, 60);
    CK(hipDeviceSynchronize());
    std::vector<float> po(60); CK(hipMemcpy(po.data(), dpo, 60*4, hipMemcpyDeviceToHost));
    bad = 0;
    ...
    float *dwv, *dwo; CK(hipMalloc(&dwv, 32*4)); CK(hipMalloc(&dwo, 4));
    CK(hipMemcpy(dwv, wv.data(), 32*4, hipMemcpyHostToDevice));
    warpmin_k<<<1,32>>>(dwv, dwo);
    CK(hipDeviceSynchronize());
    float wmin; CK(hipMemcpy(&wmin, dwo, 4, hipMemcpyDeviceToHost));
    ...
    float *dsv, *dso; CK(hipMalloc(&dsv, S*4)); CK(hipMalloc(&dso, S*4));
    CK(hipMemcpy(dsv, sv.data(), S*4, hipMemcpyHostToDevice));
    scan_k<<<1,256>>>(dsv, dso, S);
    CK(hipDeviceSynchronize());
    std::vector<float> so(S); CK(hipMemcpy(so.data(), dso, S*4, hipMemcpyDeviceToHost));
    ...
    long* dsw; CK(hipMalloc(&dsw, (long)rows*groups*8));
    swizzle_k<<<(rows*groups+255)/256,256>>>(dsw, rows, groups, num_k_tiles);
    CK(hipDeviceSynchronize());
    std::vector<long> sw((size_t)rows*groups);
    CK(hipMemcpy(sw.data(), dsw, (long)rows*groups*8, hipMemcpyDeviceToHost));
    bad = 0;
