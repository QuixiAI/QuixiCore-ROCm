...
int run(const std::string& dir, int V, int K) {
    ...
    hipMalloc(&dW, sizeof(half) * Wf.size());
    hipMalloc(&dh, sizeof(half) * T * K);
    hipMalloc(&dpv, sizeof(float) * T * num_vtiles);
    hipMalloc(&dpi, sizeof(int) * T * num_vtiles);
    hipMalloc(&dout, sizeof(int) * T);
    hipMemcpy(dWq, Wq_h.data(), Wq_h.size(), hipMemcpyHostToDevice);
    hipMemcpy(dW, Wf.data(), sizeof(half) * Wf.size(), hipMemcpyHostToDevice);
    hipMemcpy(dh, X_h.data(), sizeof(half) * T * K, hipMemcpyHostToDevice);
    ...
    for (int mode = 0; mode < 4; mode++) {           // {fp16,quant} x {argmax,categorical}
        ...
        hipMemcpy(got.data(), dout, sizeof(int) * T, hipMemcpyDeviceToHost);
...
int main(int argc, char** argv) {
    ...
    // default to gen_golden.py (V=N=512, K=4096). Override: <dir> [V K].
    char fmt[64]; int V = 512, K = 4096;
    FILE* f = fopen((dir + "/meta.txt").c_str(), "r");
    if (f) {
        if (fscanf(f, "%63s %d %d", fmt, &V, &K) != 3) return 2;
        fclose(f);
    } else {
        std::string base = dir;
        ...
        if (argc >= 4) { V = atoi(argv[2]); K = atoi(argv[3]); }
    }
