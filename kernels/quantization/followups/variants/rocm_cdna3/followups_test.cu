...
template <typename T> T* dnew(const std::vector<T>& h){T*d;CK(hipMalloc(&d,std::max<size_t>(h.size(),1)*sizeof(T)));CK(hipMemcpy(d,h.data(),h.size()*sizeof(T),hipMemcpyHostToDevice));return d;}
template <typename T> T* dz(size_t n){T*d;CK(hipMalloc(&d,std::max<size_t>(n,1)*sizeof(T)));CK(hipMemset(d,0,std::max<size_t>(n,1)*sizeof(T)));return d;}
...
static void test_tq(int k_bits, int k_signed, int v_bits, double k_tol, double v_tol) {
    ...
    if (hipGetLastError()!=hipSuccess){printf("TQ KERNEL ERR HS=%d\n",HS);++g_fail;return;}
...
static void test_apc() {
    ...
    CK(hipDeviceSynchronize());
    if (hipGetLastError()!=hipSuccess){printf("APC KERNEL ERR\n");++g_fail;return;}
...
static void test_cp_gather() {
    ...
    CK(hipDeviceSynchronize());
    if (hipGetLastError()!=hipSuccess){printf("CPGATHER KERNEL ERR\n");++g_fail;return;}
...
static void test_merge_fp8() {
    ...
    CK(hipDeviceSynchronize());
    if (hipGetLastError()!=hipSuccess){printf("MERGEFP8 KERNEL ERR\n");++g_fail;return;}
