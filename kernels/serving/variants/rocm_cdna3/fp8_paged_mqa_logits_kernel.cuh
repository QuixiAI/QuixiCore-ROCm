...
__global__ __launch_bounds__(64 * NWAVE) void fp8_paged_mqa_logits(
    ...
    long stride_q_s, long stride_w_s, long stride_logits_s,
    long block_stride, long scale_stride, int bt_stride, int block_size,
    int max_len,
    ...
    if (split_len > 0) end = end < split_begin + split_len ? end : split_begin + split_len;
...
#pragma unroll
            for (int i = 0; i < N_TILES; ++i) {
                ...
                const int n = n0 + lo;
                const long a =
                    (n < end) ? paged_a_frag<HEAD_SIZE, BLOCK_TILE, HEAD_TILE>(
