...
int main() {
    ...
    hipMalloc(&dcu, (B + 1) * 4); hipMalloc(&dqlens, B * 4); hipMalloc(&dpad, (B + 1) * 4);
    hipMalloc(&dts, max_tiles * 4); hipMalloc(&dtl, max_tiles * 4); hipMalloc(&dnt, 4);
    hipMemcpy(dcu, cu.data(), (B + 1) * 4, hipMemcpyHostToDevice);
    ...
    int nt;
    hipMemcpy(ts.data(), dts, max_tiles * 4, hipMemcpyDeviceToHost);
    hipMemcpy(tl.data(), dtl, max_tiles * 4, hipMemcpyDeviceToHost);
    hipMemcpy(pad.data(), dpad, (B + 1) * 4, hipMemcpyDeviceToHost);
    hipMemcpy(&nt, dnt, 4, hipMemcpyDeviceToHost);
    {   // reference worklist
        int bad = 0, rt = 0, rp = 0;
    ...
    const int total_padded = pad[B];
    {
        ...
        __half *dq, *dhm, *dback;
        hipMalloc(&dq, q.size() * 2); hipMalloc(&dhm, size_t(H) * total_padded * D * 2);
        hipMalloc(&dback, q.size() * 2);
        hipMemcpy(dq, q.data(), q.size() * 2, hipMemcpyHostToDevice);
        hipMemset(dback, 0, q.size() * 2);
