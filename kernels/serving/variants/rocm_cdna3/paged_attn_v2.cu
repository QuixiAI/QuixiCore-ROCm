...
int main() {
    ...
    for (int PS : {16, 48, 512}) for (int window : {0, 40}) {
        ...
        float *dtmp, *dml, *des;
        hipMalloc(&dtmp, sizeof(float) * size_t(B) * H * P * D);
        hipMalloc(&dml, sizeof(float) * B * H * P);
        hipMalloc(&des, sizeof(float) * B * H * P);
        ...
        rc |= !(maxd < 5e-3);
        hipFree(dtmp); hipFree(dml); hipFree(des);
    }
    ...
    // fp8 cache variant: quantize K/V per-KV-head, compare vs fp64 oracle on the DEQUANTIZED cache
    {
        ...
        hipMalloc(&dK8, cache_n); hipMalloc(&dV8, cache_n);
        hipMalloc(&dks, HKV * 4); hipMalloc(&dvs, HKV * 4);
        hipMemcpy(dK8, K8.data(), cache_n, hipMemcpyHostToDevice);
        hipMemcpy(dV8, V8.data(), cache_n, hipMemcpyHostToDevice);
        hipMemcpy(dks, ks.data(), HKV * 4, hipMemcpyHostToDevice);
        hipMemcpy(dvs, vs.data(), HKV * 4, hipMemcpyHostToDevice);
        ...
        rc |= !(maxd < 5e-3);
        K = Ksave; V = Vsave;
        hipFree(dK8); hipFree(dV8); hipFree(dks); hipFree(dvs);
        hipFree(dtmp); hipFree(dml); hipFree(des);
    }
    ...
    // cascade: shared prefix (len 96) + per-request paged suffix, concat partials along P
    {
        ...
        __half *dpk, *dpv;
        hipMalloc(&dpk, pk.size() * 2); hipMalloc(&dpv, pv.size() * 2);
        hipMemcpy(dpk, pk.data(), pk.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dpv, pv.data(), pv.size() * 2, hipMemcpyHostToDevice);
        float *dtmp, *dml, *des;
        hipMalloc(&dtmp, sizeof(float) * size_t(B) * H * P * D);
        hipMalloc(&dml, sizeof(float) * B * H * P);
        hipMalloc(&des, sizeof(float) * B * H * P);
        ...
        rc |= !(maxd < 5e-3);
        hipFree(dpk); hipFree(dpv); hipFree(dtmp); hipFree(dml); hipFree(des);
    }
    return rc;
}
