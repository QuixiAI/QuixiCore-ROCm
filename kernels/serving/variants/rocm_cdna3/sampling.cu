...
int main() {
    ...
    { // penalties: histogram + transform, exact elementwise
        ...
        hipMalloc(&dprev, prev.size() * 4); hipMalloc(&dcnt, size_t(T_) * V * 4);
        hipMalloc(&dpar, T_ * 4); hipMalloc(&dbias, V * 4);
        hipMalloc(&dout2, logits.size() * 4);
        hipMemcpy(dprev, prev.data(), prev.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dpar, parent.data(), T_ * 4, hipMemcpyHostToDevice);
        hipMemcpy(dbias, bias.data(), V * 4, hipMemcpyHostToDevice);
        hipMemset(dcnt, 0, size_t(T_) * V * 4);
        ...
        hipMemcpy(outp.data(), dout2, outp.size() * 4, hipMemcpyDeviceToHost);
        int bad = 0;
        for (int t = 0; t < T_; t++) {
        ...
        rc |= (bad != 0);
        hipFree(dprev); hipFree(dcnt); hipFree(dpar); hipFree(dbias); hipFree(dout2);
    }
    { // bitmask + bad words, exact
        ...
        hipMalloc(&dbm, bm.size() * 4); hipMalloc(&dbi, bad_ids.size() * 4);
        hipMalloc(&dblen, T_ * 4); hipMalloc(&do3, logits.size() * 4); hipMalloc(&do4, logits.size() * 4);
        hipMemcpy(dbm, bm.data(), bm.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dbi, bad_ids.data(), bad_ids.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dblen, bad_lens.data(), T_ * 4, hipMemcpyHostToDevice);
        ...
        hipMemcpy(o3.data(), do3, o3.size() * 4, hipMemcpyDeviceToHost);
        hipMemcpy(o4.data(), do4, o4.size() * 4, hipMemcpyDeviceToHost);
        int bad = 0;
        ...
        rc |= (bad != 0);
        hipFree(dbm); hipFree(dbi); hipFree(dblen); hipFree(do3); hipFree(do4);
    }
    return rc;
}
