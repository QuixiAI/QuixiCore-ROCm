...
int run(const char* name, void (*pack)(const float*, uint8_t*), double (*dec)(const uint8_t*, int)) {
    ...
    hipMalloc(&dK, Kq.size()); hipMalloc(&dV, Vq.size());
    hipMemcpy(dq, q.data(), q.size() * 2, hipMemcpyHostToDevice);
    hipMemcpy(dK, Kq.data(), Kq.size(), hipMemcpyHostToDevice);
    hipMemcpy(dV, Vq.data(), Vq.size(), hipMemcpyHostToDevice);
    ...
    hipFree(dq); hipFree(dout); hipFree(dK); hipFree(dV);
    return rc;
}
