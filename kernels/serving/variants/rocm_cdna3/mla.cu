...
int main() {
    ...
    for (int D : {192, 512}) {
        ...
        hipMalloc(&dq, q.size() * 2); hipMalloc(&dW, D * 2);
        hipMalloc(&dc, cb.size() * 2); hipMalloc(&ds, sb.size() * 2);
        hipMalloc(&dout, q.size() * 2); hipMalloc(&dpos, T_ * 4);
        hipMemcpy(dq, q.data(), q.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dW, W.data(), D * 2, hipMemcpyHostToDevice);
        hipMemcpy(dc, cb.data(), cb.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(ds, sb.data(), sb.size() * 2, hipMemcpyHostToDevice);
        ...
        hipFree(dq); hipFree(dW); hipFree(dc); hipFree(ds); hipFree(dout); hipFree(dpos);
    }
    ...
    // ---- 2. bf16 absorb decode <512,64> vs fp64 oracle ----
    {
        ...
        hipMalloc(&dc_, cache.size() * 2); hipMalloc(&dq, q.size() * 2);
        hipMalloc(&dout, size_t(B) * H * LAT * 2);
        hipMalloc(&dbt, bt.size() * 4); hipMalloc(&dctx, B * 4);
        hipMemcpy(dc_, cache.data(), cache.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dq, q.data(), q.size() * 2, hipMemcpyHostToDevice);
        ...
        hipMemcpy(got.data(), dout, got.size() * 2, hipMemcpyDeviceToHost);
        double maxd = 0;
        ...
        rc |= !(maxd < 6e-3);
        hipFree(dc_); hipFree(dq); hipFree(dout); hipFree(dbt); hipFree(dctx);
    }
    ...
    // ---- 3. fp8 insert -> fp8 decode round trip vs oracle over the dequantized cache ----
    {
        ...
        hipMalloc(&ddata, size_t(nb) * BS * 576); hipMalloc(&dscale, size_t(nb) * BS * 8);
        hipMalloc(&dkv, kv.size() * 2); hipMalloc(&dcb, cb.size() * 2); hipMalloc(&dsb, sb.size() * 2);
        hipMalloc(&dpos, T_ * 4); hipMalloc(&dslot, T_ * 8);
        hipMemcpy(dkv, kv.data(), kv.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dcb, cb.data(), cb.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dsb, sb.data(), sb.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dpos, pos.data(), T_ * 4, hipMemcpyHostToDevice);
        ...
        hipMemcpy(data.data(), ddata, data.size(), hipMemcpyDeviceToHost);
        hipMemcpy(scl.data(), dscale, scl.size(), hipMemcpyDeviceToHost);
        auto dec = [](uint8_t v) {
            float mag;
        ...
        rc |= !(maxd < 6e-3);
        hipFree(ddata); hipFree(dscale); hipFree(dkv); hipFree(dcb); hipFree(dsb);
        hipFree(dpos); hipFree(dslot); hipFree(dq); hipFree(dout); hipFree(dbt); hipFree(dctx);
    }
    // ---- 4. bf16 mla_kv_insert (norm modes 0/2, neg slot skip) vs fp64 replay ----
    {
        ...
        hipMalloc(&dkvc, kvc.size() * 2); hipMalloc(&dkpe, kpe.size() * 2);
        hipMalloc(&dW, LAT * 2); hipMalloc(&dcb, cb.size() * 2); hipMalloc(&dsb, sb.size() * 2);
        hipMalloc(&dcache, cache_n * 2); hipMalloc(&dpos, T_ * 4); hipMalloc(&dslot, T_ * 8);
        hipMemcpy(dkvc, kvc.data(), kvc.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dkpe, kpe.data(), kpe.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dW, W.data(), LAT * 2, hipMemcpyHostToDevice);
        hipMemcpy(dcb, cb.data(), cb.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dsb, sb.data(), sb.size() * 2, hipMemcpyHostToDevice);
        hipMemcpy(dpos, pos.data(), T_ * 4, hipMemcpyHostToDevice);
        hipMemcpy(dslot, slots.data(), T_ * 8, hipMemcpyHostToDevice);
        for (int mode : {0, 2}) {
            ...
            hipMemcpy(dcache, init.data(), cache_n * 2, hipMemcpyHostToDevice);
            mla_kv_insert<512><<<T_, 32>>>(dkvc, dkpe, dcb, dsb, dpos, dslot, dcache,
                                           BS, ROPE, mode, 1e-6f, dW);
            hipDeviceSynchronize();
            std::vector<bf16> got(cache_n);
            hipMemcpy(got.data(), dcache, cache_n * 2, hipMemcpyDeviceToHost);
            double maxd = 0;
            bool skip_ok = true;
        ...
        hipFree(dkvc); hipFree(dkpe); hipFree(dW); hipFree(dcb); hipFree(dsb);
        hipFree(dcache); hipFree(dpos); hipFree(dslot);
    }
    ...
    // ---- 5. partitioned + sparse decode variants vs fp64 oracles ----
    {
        ...
        int *didx, *dtlen;
        hipMalloc(&didx, idx.size() * 4); hipMalloc(&dtlen, B * 4);
        hipMemcpy(didx, idx.data(), idx.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dtlen, tlen.data(), B * 4, hipMemcpyHostToDevice);
        ...
        float *dtmp, *dml, *des;
        hipMalloc(&dtmp, size_t(B) * H * P * LAT * 4);
        hipMalloc(&dml, size_t(B) * H * P * 4); hipMalloc(&des, size_t(B) * H * P * 4);
        ...
        hipMalloc(&dtmp2, size_t(B) * H * P2 * LAT * 4);
        hipMalloc(&dml2, size_t(B) * H * P2 * 4); hipMalloc(&des2, size_t(B) * H * P2 * 4);
        ...
        // (d) bf16 partitioned decode + reduce vs the serial bf16 kernel's oracle path
        {
            ...
            bf16 *dc_, *dq2, *dout2;
            hipMalloc(&dc_, cache.size() * 2); hipMalloc(&dq2, qq.size() * 2);
            hipMalloc(&dout2, size_t(B) * H * LAT * 2);
            hipMemcpy(dc_, cache.data(), cache.size() * 2, hipMemcpyHostToDevice);
            hipMemcpy(dq2, qq.data(), qq.size() * 2, hipMemcpyHostToDevice);
            ...
            hipMemcpy(got2.data(), dout2, got2.size() * 2, hipMemcpyDeviceToHost);
            double md2 = 0;
            for (int b = 0; b < B; b++) for (int h = 0; h < H; h++) {
                std::vector<double> sc; std::vector<int64_t> bases;
                for (int t = 0; t < ctx[b]; t++) {
            ...
            rc |= !(md2 < 6e-3);
            hipFree(dc_); hipFree(dq2); hipFree(dout2);
        }
        hipFree(ddata); hipFree(dscale); hipFree(dkv); hipFree(dcb); hipFree(dsb);
        hipFree(dpos); hipFree(dslot); hipFree(dq); hipFree(dout); hipFree(dbt); hipFree(dctx);
        hipFree(didx); hipFree(dtlen); hipFree(dtmp); hipFree(dml); hipFree(des);
        hipFree(dtmp2); hipFree(dml2); hipFree(des2);
    }
    return rc;
}
