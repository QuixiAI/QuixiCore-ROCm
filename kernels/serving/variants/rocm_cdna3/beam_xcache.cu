...
int main() {
    ...
    // ---- beam builders vs CPU reference ----
    {
        ...
        hipMalloc(&dp, parent.size() * 4); hipMalloc(&dbt, bt.size() * 4);
        hipMalloc(&dseq, seq.size() * 4); hipMalloc(&dpairs, size_t(n_slots) * 16);
        hipMalloc(&dnbt, bt.size() * 4);
        hipMemcpy(dp, parent.data(), parent.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dbt, bt.data(), bt.size() * 4, hipMemcpyHostToDevice);
        hipMemcpy(dseq, seq.data(), seq.size() * 4, hipMemcpyHostToDevice);
        ...
        hipMemcpy(pairs.data(), dpairs, pairs.size() * 8, hipMemcpyDeviceToHost);
        hipMemcpy(nbt.data(), dnbt, nbt.size() * 4, hipMemcpyDeviceToHost);
        int bad = 0;
        ...
        rc |= (bad != 0);
        hipFree(dp); hipFree(dbt); hipFree(dseq); hipFree(dpairs); hipFree(dnbt);
    }
    ...
    // ---- kv_cache_scales ----
    {
        ...
        hipMalloc(&dk, n * 2); hipMalloc(&dv, n * 2);
        hipMalloc(&dks, 4); hipMalloc(&dvs, 4);
        hipMemcpy(dk, k.data(), n * 2, hipMemcpyHostToDevice);
        hipMemcpy(dv, v.data(), n * 2, hipMemcpyHostToDevice);
        kv_cache_scales<__half><<<1, 256>>>(dk, dv, dks, dvs, n);
        hipDeviceSynchronize();
        float ks, vs;
        hipMemcpy(&ks, dks, 4, hipMemcpyDeviceToHost);
        hipMemcpy(&vs, dvs, 4, hipMemcpyDeviceToHost);
        const bool ok = (ks == km / 240.0f) && (vs == vm / 240.0f);
        printf("kv_cache_scales: %s\n", ok ? "PASS" : "FAIL");
        rc |= !ok;
        hipFree(dk); hipFree(dv); hipFree(dks); hipFree(dvs);
    }
    ...
    // ---- xcache decode vs fp64 oracle (x = 16/sizeof(half) = 8) ----
    {
        ...
        hipMalloc(&dK, K.size() * 2); hipMalloc(&dV, V.size() * 2);
        ...
        rc |= !(maxd < 5e-3);
        hipFree(dK); hipFree(dV); hipFree(dq); hipFree(dout); hipFree(dbt); hipFree(dctx);
    }
    return rc;
}
