...
aiter = None
if os.environ.get("USE_AITER_REF") == "1":
    try:
        import aiter
    except Exception as exc:
        print(f"AITer import failed; using PyTorch SDPA reference: {exc!r}")
else:
    print("USE_AITER_REF=1 not set; using PyTorch SDPA reference.")
...
def sdpa_reference(q, k, v):
    ...
    return scaled_dot_product_attention(q_ref, k_ref, v_ref, is_causal=bool(causal)).transpose(1, 2).contiguous()
...
def reference_attention(q, k, v):
    ...
    if aiter is not None:
        try:
            ...
            return out_ref, lse_ref, "AITER"
        except Exception as exc:
            print(f"AITer reference failed; using PyTorch SDPA reference: {exc!r}")
            aiter = None
    return sdpa_reference(q, k, v), None, "PyTorch SDPA"
...
print("Reference:")
out_ref, lse_ref, ref_backend = reference_attention(q, k, v)
if ref_backend == "AITER":
    ...
    print(f"AITER reference average execution time: {avg_time_ref:.4f} ms")
    print(f"AITER reference performance: {eff_ref:.2f} TFLOPS for {B=} {H=} {N=} {D=} {causal=}.\n")
else:
    print("Using PyTorch SDPA reference for correctness; AITer timing skipped.\n")
...
num_print = 16
print(f"\n TK vs {ref_backend} comparison:")
print("\nO outputs:")
print("TK: ", out[0, 0, :num_print, 0], "Max:", out.max().item())
print(f"{ref_backend}: ", out_ref[0, 0, :num_print, 0], "Max:", out_ref.max().item())
