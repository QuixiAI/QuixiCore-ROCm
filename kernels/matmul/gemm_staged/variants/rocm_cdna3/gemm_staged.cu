/**
 * @file
 * @brief CDNA3 LDS-staged GEMM port for the Metal gemm_staged contract.
 */
...
__global__ void gemm_staged_tiled(T* __restrict__ C,
    ...
    for (int k0 = 0; k0 < K; k0 += TILE) {
        As[ty][tx] = (row < N && k0 + tx < K) ? A[size_t(row) * K + k0 + tx] : ft<T>(0.0f);
        Bs[ty][tx] = (k0 + ty < K && col < M) ? B[size_t(k0 + ty) * M + col] : ft<T>(0.0f);
        __syncthreads();
        #pragma unroll
        for (int k = 0; k < TILE; ++k) acc += tf(As[ty][k]) * tf(Bs[k][tx]);
        __syncthreads();
    }
    if (row < N && col < M) C[size_t(row) * M + col] = ft<T>(acc);
}
...
int run_one<float>(const char* name, const std::vector<float>& Af, const std::vector<float>& Bf,
    ...
    auto launch = [&] { gemm_staged_tiled<float, 16><<<grid, block>>>(dC, dA, dB, N, K, M); };
    launch();
    CK(hipDeviceSynchronize());
    CK(hipGetLastError());
...
int run_one<__hip_bfloat16>(const char* name, const std::vector<float>& Af, const std::vector<float>& Bf,
    ...
    dim3 block(16, 16), grid((M + 15) / 16, (N + 15) / 16);
    auto launch = [&] { gemm_staged_tiled<__hip_bfloat16, 16><<<grid, block>>>(dC, dA, dB, N, K, M); };
    launch();
    CK(hipDeviceSynchronize());
    CK(hipGetLastError());
    auto C = d2h(dC, size_t(N) * M);
...
int main(int argc, char** argv) {
    ...
    int rc = 0;
    rc |= run_one<float>("gemm_staged", A, B, N, K, M, bench);
    rc |= run_one<__hip_bfloat16>("gemm_staged", A, B, N, K, M, bench);
    return rc;
}
