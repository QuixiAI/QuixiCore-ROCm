...
namespace detail {
template<typename T>
struct is_rv : std::false_type {};

template<typename T, size_t length, typename layout>
struct is_rv<rv<T, length, layout>> : std::true_type {};
} // namespace detail
...
*/
template<typename T>
concept all = kittens::detail::is_rv<std::remove_cvref_t<T>>::value;
