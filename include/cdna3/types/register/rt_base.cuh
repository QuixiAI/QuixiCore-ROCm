 ...
 */
template<typename _T, ducks::rt_layout::all _layout, ducks::rt_shape::all _shape=ducks::rt_shape::rt_16x16>
struct rt_base {
    ...
    static constexpr int num_packed           = sizeof(dtype) / sizeof(T);
    static constexpr int packed_per_thread    = elements_per_thread / num_packed;
    static constexpr int stride               = tile_size_col;
    static constexpr int packed_per_stride    = stride / num_packed;
    static constexpr int num_strides          = packed_per_thread / packed_per_stride;
...
namespace detail {
template<typename T>
struct is_rt_base : std::false_type {};

template<typename T, typename L, typename S>
struct is_rt_base<rt_base<T, L, S>> : std::true_type {};
} // namespace detail
...
*/
template<typename T> concept all = kittens::detail::is_rt_base<std::remove_cvref_t<T>>::value;
} // namespace rt
} // namespace ducks
