...
namespace detail {
template<typename T, typename Enable = void> struct size_info {
    static constexpr uint32_t bytes    = sizeof(std::remove_reference_t<T>);
};
template<typename ST> struct size_info<ST, std::enable_if_t<kittens::detail::is_st<std::remove_cvref_t<ST>>::value>> {
...
template<typename SV> struct size_info<SV, std::enable_if_t<kittens::detail::is_sv<std::remove_cvref_t<SV>>::value>> {
