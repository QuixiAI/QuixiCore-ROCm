 ...
 * @tparam SV The type of the shared memory vector. Must satisfy the `kittens::ducks::sv::all` concept.
 ...
 */
template<typename op, typename SV, bool reset, std::enable_if_t<kittens::detail::is_sv<std::remove_cvref_t<SV>>::value, int> = 0>
 ...
 */
template<typename SV, std::enable_if_t<kittens::detail::is_sv<std::remove_cvref_t<SV>>::value, int> = 0>
