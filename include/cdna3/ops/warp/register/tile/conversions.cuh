 ...
 */
template<typename T, kittens::ducks::rt_layout::all layout>
__device__ inline void swap_layout(rt_base<T, typename kittens::ducks::rt_layout::transpose<layout>::type> &dst, const rt_base<T, layout> &src) {
    int lane = laneid();
 ...
 */
template<typename T2, int _height, int _width, kittens::ducks::rt_layout::all layout>
__device__ static inline void swap_layout(rt<T2, _height, _width, typename kittens::ducks::rt_layout::transpose<layout>::type> &dst, const rt<T2, _height, _width, layout> &src) {
            ...
            if constexpr (std::is_same_v<shape1, typename ducks::rt_shape::rt_16x32> && std::is_same_v<shape2, typename ducks::rt_shape::rt_16x16>) {
                ...
                for (int i = 0; i < dst.height; i++) {
                    ...
                    for (int j = 0; j < dst.width; j++) {
                        #pragma unroll
                        for (int k = 0; k < 2; k++) {
                            dst.tiles[i][j].data[k] = src.tiles[i][j * 2].data[k];
                            dst.tiles[i][j].data[k + 2] = src.tiles[i][j * 2 + 1].data[k];
                        }
                    }
                }
                ...
                for (int i = 0; i < dst.height; i++) {
                    ...
                    for (int j = 0; j < dst.width; j++) {
                        ...
                        for (int k = 0; k < dst.tiles[i][j].packed_per_thread; k++) {
                            dst.tiles[i][j].data[k] = src.tiles[i / 2][j].data[(i % 2) * dst.tiles[i][j].packed_per_thread + k];
                        }
                    }
                }
            } else {
 ...
 */
template<typename T2, kittens::ducks::rt_layout::all layout>
__device__ inline rt_base<T2, typename kittens::ducks::rt_layout::transpose<layout>::type>& swap_layout_inplace(const rt_base<T2, layout> &src) {
    rt_base<T2, typename kittens::ducks::rt_layout::transpose<layout>::type> &dst = *(rt_base<T2, typename kittens::ducks::rt_layout::transpose<layout>::type>*)(&src);
    swap_layout(dst, src);
    return dst;
}
 ...
 */
template<typename T2, int _rows, int _cols, kittens::ducks::rt_layout::all layout>
__device__ static inline rt<T2, _rows, _cols, typename kittens::ducks::rt_layout::transpose<layout>::type>& swap_layout_inplace(rt<T2, _rows, _cols, layout> &tile) {
    #pragma unroll
    ...
    return *(rt<T2, _rows, _cols, typename kittens::ducks::rt_layout::transpose<layout>::type>*)(&tile);
}
 ...
 */
template<typename T, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape=kittens::ducks::rt_shape::rt_16x16>
...
template<typename T2, int rows, int cols, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape>
__device__ static inline void transpose(
    rt<T2, cols, rows, typename kittens::ducks::rt_layout::transpose<layout>::type, typename kittens::ducks::rt_shape::transpose<shape>::type> &dst,
    const rt<T2, rows, cols, layout, shape> &src
) {
    #pragma unroll
    for (int i = 0; i < src.height; i++) {
        ...
        for (int j = 0; j < src.width; j++) {
            ...
            for (int k = 0; k < src.packed_per_tile; k++) {
                __builtin_memcpy(&dst.tiles[j][i].data[k], &src.tiles[i][j].data[k], sizeof(src.tiles[i][j].data[k]));
            }
        }
    }
}
 ...
 */
template<typename RT, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<RT>>::value, int> = 0>
__device__ static inline void transpose_sep(RT &dst, const rt<typename RT::T, RT::cols, RT::rows, typename RT::layout, typename RT::shape> &src) {
    #pragma unroll
 ...
 */
template<typename T2, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape=kittens::ducks::rt_shape::rt_16x16>
 ...
 */
template<typename T2, int _rows, int _cols, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape=kittens::ducks::rt_shape::rt_16x16>
 ...
 */
template<typename T, typename U, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape=kittens::ducks::rt_shape::rt_16x16>
 ...
 */
template<typename T2, typename U2, int _height, int _width, kittens::ducks::rt_layout::all layout, kittens::ducks::rt_shape::all shape=kittens::ducks::rt_shape::rt_16x16>
 ...
 */
template<typename RT, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<RT>>::value && std::is_same_v<typename std::remove_cvref_t<RT>::layout, kittens::ducks::rt_layout::row>, int> = 0>
...
template<typename RT, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<RT>>::value && std::is_same_v<typename std::remove_cvref_t<RT>::layout, kittens::ducks::rt_layout::col>, int> = 0>
__device__ static inline void tril(RT &dst, const RT &src, const int row_idx, const typename base_types::packing<typename RT::dtype>::unpacked_type &val=0) {
...
* @tparam RT The type of the input tile, which must satisfy the typename concept, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<concept>>::value, int> = 0.
...
*/
template<int subtile_rows, typename RT, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<RT>>::value, int> = 0>
