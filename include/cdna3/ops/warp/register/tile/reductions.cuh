 ...
 */
template<typename op, typename V, typename T, bool reset, std::enable_if_t<kittens::detail::is_rv<std::remove_cvref_t<V>>::value && kittens::detail::is_rt<std::remove_cvref_t<T>>::value && std::is_same_v<typename std::remove_cvref_t<T>::layout, kittens::ducks::rt_layout::row>, int> = 0>
 ...
 * @param[in] src_accum The initial value of the accumulator, used when reset is false.
 */
template<typename op, typename V, typename T, bool reset, std::enable_if_t<kittens::detail::is_rv<std::remove_cvref_t<V>>::value && kittens::detail::is_rt<std::remove_cvref_t<T>>::value && std::is_same_v<typename std::remove_cvref_t<T>::layout, kittens::ducks::rt_layout::col>, int> = 0>
__device__ static inline void row_reduce(V &row_accum, const T &src, const V &src_accum) {
    // I actually like these static asserts because they give more verbose errors when things go wrong.
    static_assert(std::is_same_v<typename V::layout, typename rt_base<typename T::T, typename T::layout>::col_vec_layout>); // compatible layout
 ...
 * @param[in] src The source matrix on which to perform the reduction.
 */
template<typename V, std::enable_if_t<kittens::detail::is_rv<std::remove_cvref_t<V>>::value, int> = 0, typename T, std::enable_if_t<kittens::detail::is_rt<std::remove_cvref_t<T>>::value, int> = 0>
