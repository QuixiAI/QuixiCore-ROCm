 ...
 * @tparam RV The type of the register vector. Must satisfy the `kittens::ducks::rv::all` concept.
 ...
 */
template<typename op, typename RV, bool reset, std::enable_if_t<kittens::detail::is_rv<std::remove_cvref_t<RV>>::value, int> = 0>
 ...
 */
template<typename RV, std::enable_if_t<kittens::detail::is_rv<std::remove_cvref_t<RV>>::value, int> = 0>
